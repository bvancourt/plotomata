...
ListColor: TypeAlias = list[
    float, float, float, float
]  # [r, g, b, a]; may occur when getting colors form R
...
mpl_tab20 = plt.get_cmap("tab20")
tab20_order = np.array([i * 2 for i in range(10)] + [i * 2 + 1 for i in range(10)])
tab20_colors: dict[int, Color] = {
    i_new: mpl_tab20(i_old) for i_new, i_old in enumerate(tab20_order)
}
...
nb50_order = np.arange(1, 51) % 50
nb50_colors: dict[int, Color] = {
    i_new: (npy_nb50[i_old, 0], npy_nb50[i_old, 1], npy_nb50[i_old, 2], 1.0)
    for i_new, i_old in enumerate(nb50_order)
}
    ...
    **kwargs: dict[str, Union[str, float, None]],
    ...
    if mode == "stacked":
        ...
        for i_layer, label in enumerate(data_df.index):
            heights = [
                data_df[col_key][i_layer] for col_key, bottom in zip(col_list, bottoms)
            ]
            ax.bar(
                col_offsets,
                heights,
...
def column_plot(
    ...
    shaded_range: ShadedRangeType = None,
